...
def _normalize_paths(value: object, repo: Path) -> object:
    ...
    if isinstance(value, str):
        return (
            value.replace(private_resolved, "<REPO>").replace(repo_resolved, "<REPO>")
        )
    return value
...
def _run_case_managed_e2e(repo: Path, fixture: dict) -> dict:
    ...
    init_result = _run_json(
        [
            ...
            "--goal",
            "Expose a provider health view in settings",
            "--problem",
            "Users cannot verify provider readiness before a request fails",
            "--scope",
            "Settings UI, health command, relevant docs",
            "--non-goals",
            "Historical analytics and background polling",
            "--constraint",
            "Respect existing Tauri command boundaries",
            "--surface",
            "apps/web",
            "--validation",
            "Manual settings verification and targeted tests",
            "--doc",
            "docs/OBSERVABILITY.md",
            "--assumption",
            "Existing provider config path remains the source of truth",
            "--risk",
            "UI may misrepresent transient provider failures if health semantics are weak",
            "--date",
            "2026-04-05",
        ]
    )
    archive_result = _run_json(
        [
            ...
            "--summary",
            "Delivered provider health planning loop",
            "--duration",
            "1 day",
            "--learning",
            "Keep plan lifecycle script-driven",
            "--completed-date",
            "2026-04-06",
        ]
    )
...
def _run_case_unmanaged_plans_safe(repo: Path, fixture: dict) -> dict:
    ...
    init_result = _run_json(
        [
            ...
            str(repo),
            "--title",
            "Search ranking tweak",
            "--goal",
            "Improve search ordering",
            "--problem",
            "Current ordering hides exact matches",
            "--scope",
            "Ranking logic only",
            "--validation",
            "Targeted ranking assertions",
            "--date",
            "2026-04-05",
        ]
    )
...
def _run_case(case_name: str, update_golden: bool) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix=f"harness-feat-{case_name}-") as temp_dir:
