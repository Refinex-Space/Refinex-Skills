...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Initialize a feature execution plan")
    parser.add_argument("--repo", default=".", help="Repository root")
    parser.add_argument("--title", required=True, help="Plan title")
    parser.add_argument("--goal", help="Desired outcome")
    parser.add_argument("--problem", help="Why the work exists")
    parser.add_argument("--scope", help="Included scope")
    parser.add_argument("--non-goals", dest="non_goals", help="Excluded scope")
    parser.add_argument("--rollback", help="Rollback path")
    ...
    parser.add_argument("--surface", action="append", default=[], help="Affected surface, repeatable")
    ...
    parser.add_argument("--assumption", action="append", default=[], help="Open assumption, repeatable")
    parser.add_argument("--risk", action="append", default=[], help="Main risk, repeatable")
...
def default_validation_note(language: str) -> str:
    return "初始化计划，待补充实际验证命令与结果" if language == "zh" else "Plan initialized; add real validation commands and outcomes"
...
def render_content(args: argparse.Namespace, plan_path: Path, repo_root: Path, language: str) -> str:
    ...
    template = load_template(Path(__file__).resolve().parent.parent, f"feature-plan.{language}.md.tpl")
    return template.substitute(
        title=args.title.strip(),
        owner=args.owner.strip(),
        date=args.date,
        goal=normalize_text(args.goal, language),
        problem=normalize_text(args.problem, language),
        scope=normalize_text(args.scope, language),
        non_goals=normalize_text(args.non_goals, language),
        rollback=normalize_text(args.rollback, language) if args.rollback else default_rollback(language),
        plan_path=relative_path,
        constraints=format_block(args.constraint, language),
        affected_surfaces=format_block(args.surface, language),
        validation=format_block(args.validation, language),
        docs_sync=format_block(docs_sync_items, language),
        open_assumptions=format_block(args.assumption, language),
...
def main() -> int:
    ...
    slug = normalize_slug(raw_slug, "feat", args.date.replace("-", ""))
