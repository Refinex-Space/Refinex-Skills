...
If the central claim depends on a mechanism, branching flow, lifecycle, or structural relationship that is hard to keep in working memory, the body includes a diagram chosen with `shared-diagram-selection-guide.md`. A strong blog post is allowed to use one or two hard-working diagrams; it is not restricted to prose-only just because it is "a blog post".
