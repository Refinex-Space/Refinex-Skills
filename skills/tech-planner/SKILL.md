...
The second is **title theater**, where article titles either collapse into topic tags or over-expand into AI-scented rhetorical constructions. "Introduction to Spring AI" is a topic tag; "ChatClient → AdvisorChain → ChatModel 三层夹心——你的第一次 API 调用背后发生了什么" is overloaded in the opposite direction. A strong title makes one clean claim in editorial language. It can be sharp without sounding theatrical.

The third is **phase taxonomy theater**, where phases are named either as generic difficulty buckets ("Basics", "Intermediate", "Advanced") or as inflated abstractions ("认知重建", "机制精通", "能力扩展") that sound more like generated taxonomy than editorial judgment. A strong phase name is short, natural, and specific enough to orient the reader. The phase goal paragraph then carries the fuller cognitive move.
