...
Each node carries metadata with it. The planner records the node's name, a one-sentence definition in the planner's own words (not the documentation's words), the source from which it came, a one-sentence note on why the concept matters, and a one-sentence note on what the reader can do once the concept is understood. The "why it matters" note is the seed of the eventual article's central argument; the "what the reader can do" note is the seed of the learning ramp. Writing both down at the node-extraction step forces the planner to articulate the concept's purpose rather than leaving it implicit.
...
The third step groups nodes into clusters that will eventually become articles. A cluster is a set of nodes that belong together in a single article — they are conceptually related, share prerequisites, unlock a coherent reader benefit, and can be introduced together without overwhelming the reader.

The clustering uses four heuristics. The first is shared prerequisites: nodes that share the same set of prerequisite nodes are usually in the same cluster, because they become available to the reader at the same point in the series and can be introduced as a group. The second is conceptual proximity: nodes that are about the same aspect of the framework — say, all the nodes about error handling, or all the nodes about extension points — are usually in the same cluster, even when their prerequisites differ. The third is reader benefit: nodes that together unlock one concrete capability ("can boot a correct project", "can build a basic RAG pipeline", "can diagnose one specific failure class") are usually better clustered together than nodes that are merely adjacent in the documentation. The fourth is cognitive load: a single article can introduce roughly one cluster of new complexity without overwhelming the reader, so clusters should be sized to fit within an article's cognitive budget.
...
The output of Step 3 is the list of clusters, each with its member nodes, its prerequisite clusters (computed from the prerequisite edges between clusters), a one-sentence description of what the cluster covers, and a one-sentence description of the concrete capability the reader gains after that article.
