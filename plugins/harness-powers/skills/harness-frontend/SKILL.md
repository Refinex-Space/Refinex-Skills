...
1. **Repository mode** — the task changes a real codebase. `harness-brainstorm`, `harness-feat`, or `harness-fix` must already own the workflow. This skill may not bypass design approval, preflight, planning, or fresh verification evidence.
...
1. Hand control back to the owning process skill (`harness-brainstorm`, `harness-feat`, or `harness-fix`).
