...
Scan `docs/exec-plans/specs/`:

- Every spec linked from an active plan must exist
- Specs without a corresponding active or completed plan should be reviewed for abandonment
- Specs that still mention legacy Superpowers default paths should be flagged
- Specs older than 30 days with no plan should be reviewed with the user
...
- Links to specs under `exec-plans/specs/` should resolve

Flag stale Harness Powers path references to the old `docs/superpowers/` spec or plan directories.
