...
Harness Powers 是一个面向 Codex App 的 agent-first 软件开发插件，维护在本仓库的 `plugins/harness-powers/` 目录下。它把 Harness Engineering 的仓库控制面能力和 Superpowers 的 agent 执行能力整合到同一套 Harness 生命周期里。
...
插件结构遵循 Codex plugin 官方规则：`.codex-plugin/plugin.json` 是必需入口；`skills/`、`assets/` 等目录保留在 plugin root；manifest 里的路径必须相对 plugin root，并且 `source.path` 必须以 `./` 开头。
