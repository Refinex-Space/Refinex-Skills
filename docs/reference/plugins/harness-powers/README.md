...
Harness Powers 是一个面向 Codex App 和 Cursor 的 agent-first 软件开发插件，维护在本仓库的 `plugins/harness-powers/` 目录下。它把 Harness Engineering 的仓库控制面能力和 Superpowers 的 agent 执行能力整合到同一套 Harness 生命周期里。
...
plugins/harness-powers/.cursor-plugin/plugin.json
plugins/harness-powers/README.md
plugins/harness-powers/skills/
plugins/harness-powers/assets/harness-powers-small.svg
```

Codex manifest 与 Cursor manifest 中的 `skills` 字段都指向 `./skills/`，安装后会从这个目录发现 `harness-*` skills。

Codex plugin 入口是 `.codex-plugin/plugin.json`；Cursor plugin 入口是 `.cursor-plugin/plugin.json`。`skills/`、`assets/` 等目录保留在 plugin root；manifest 里的路径必须相对 plugin root。Codex marketplace 的 `source.path` 还必须以 `./` 开头。

官方参考：

- [Build plugins](https://developers.openai.com/codex/plugins/build)
- [Plugins](https://developers.openai.com/codex/plugins)
- [Cursor Plugins](https://cursor.com/cn/docs/plugins)
- [Cursor Plugins Reference](https://cursor.com/docs/reference/plugins)

### 推荐安装方式：Repo Marketplace

按官方文档，本地 plugin 需要通过 marketplace 暴露给 Codex。面向这个仓库使用时，在仓库根目录创建或更新以下文件：

```text
.agents/plugins/marketplace.json
```

内容示例（`source.path` 与平台无关，始终使用 `./` 相对路径）：
...
cp -R /path/to/Refinex-Skills/plugins/harness-powers ~/.codex/plugins/harness-powers
```

然后创建或更新 `~/.agents/plugins/marketplace.json`：
...
```powershell
# 创建个人插件目录
New-Item -ItemType Directory -Force -Path "$env:USERPROFILE\.codex\plugins"

# 复制插件
Copy-Item -Recurse -Force `
  "C:\path\to\Refinex-Skills\plugins\harness-powers" `
  "$env:USERPROFILE\.codex\plugins\harness-powers"
```

然后创建或更新 `%USERPROFILE%\.agents\plugins\marketplace.json`（PowerShell）：
...
> **Windows 路径说明：**
> - 个人 home 目录为 `%USERPROFILE%`，对应 PowerShell 中的 `$env:USERPROFILE`，通常为 `C:\Users\<username>`。
> - marketplace 文件路径：`%USERPROFILE%\.agents\plugins\marketplace.json`。
> - 插件缓存路径：`%USERPROFILE%\.codex\plugins\cache\`。
> - Codex 配置文件路径：`%USERPROFILE%\.codex\config.toml`。 
> - `source.path` 中始终使用 `./` 相对路径，Codex 会自行处理跨平台路径解析，无需写成 Windows 反斜杠格式。
...
```bash
codex plugin marketplace add /path/to/Refinex-Skills
```

**Windows（PowerShell）：**

```powershell
codex plugin marketplace add "C:\path\to\Refinex-Skills"
```

如果 marketplace 来自远端仓库，也可以使用 GitHub shorthand、Git URL、`--ref` 或 `--sparse`（跨平台相同）：

```bash
codex plugin marketplace add Refinex-Space/Refinex-Skills
```

更新或移除 marketplace（跨平台相同）：
...
```text
%USERPROFILE%\.codex\plugins\cache\<MARKETPLACE_NAME>\<PLUGIN_NAME>\<VERSION>
```

对本地 plugin，`$VERSION` / `<VERSION>` 通常是 `local`。因此修改源目录后，需要更新 marketplace 指向的 plugin 内容并重启 Codex，必要时重新安装或刷新 marketplace，才能让本地修改生效。
...
Cursor 官方插件结构要求插件根目录包含：

```text
.cursor-plugin/plugin.json
```

Harness Powers 已在同一个 plugin root 下提供 Cursor manifest：
...
推荐用符号链接做本地安装，方便继续在仓库内迭代：

```bash
mkdir -p ~/.cursor/plugins/local
ln -s /path/to/Refinex-Skills/plugins/harness-powers ~/.cursor/plugins/local/harness-powers
```

如果目标路径已经存在，先确认它是否是旧版本或旧链接，再删除或改名后重新创建链接。

也可以复制插件目录：

```bash
mkdir -p ~/.cursor/plugins/local
cp -R /path/to/Refinex-Skills/plugins/harness-powers ~/.cursor/plugins/local/harness-powers
```

复制方式不会自动同步仓库后续修改；修改源码后需要重新复制，或改用符号链接。
...
Copy-Item -Recurse -Force `
  "C:\path\to\Refinex-Skills\plugins\harness-powers" `
  "$env:USERPROFILE\.cursor\plugins\local\harness-powers"
```

如果已启用开发者模式并允许创建符号链接，也可以使用目录符号链接：

```powershell
New-Item -ItemType Directory -Force -Path "$env:USERPROFILE\.cursor\plugins\local"
New-Item -ItemType SymbolicLink `
  -Path "$env:USERPROFILE\.cursor\plugins\local\harness-powers" `
  -Target "C:\path\to\Refinex-Skills\plugins\harness-powers"
```

安装后重启 Cursor，或执行 `Developer: Reload Window`。然后在 Cursor 设置的 Rules/Skills 区域确认 `harness-*` skills 已加载。

### Cursor 使用前 Smoke Test

Cursor 中的 skill 手动触发使用 `/skill-name` 形式。安装并 reload 后，在 Cursor Chat 中发起最小验证：

```text
/harness-using 检查当前仓库任务应该进入哪条 Harness Powers 流程。
```

如果 `/harness-using` 无法解析，优先检查：

- `~/.cursor/plugins/local/harness-powers` 或 `%USERPROFILE%\.cursor\plugins\local\harness-powers` 是否存在。
- 插件根目录下是否存在 `.cursor-plugin/plugin.json`。
- `.cursor-plugin/plugin.json` 中 `skills` 是否指向 `./skills/`。
- 是否已经重启 Cursor 或执行 `Developer: Reload Window`。
- Cursor 设置的 Rules/Skills 区域是否显示对应 skill。

### Codex 使用前 Smoke Test
...
- marketplace 文件是否位于 `$REPO_ROOT/.agents/plugins/marketplace.json` 或个人目录下（macOS/Linux：`~/.agents/plugins/marketplace.json`；Windows：`%USERPROFILE%\.agents\plugins\marketplace.json`）。
- `source.path` 是否以 `./` 开头，并且指向 plugin 目录。
- 是否已经重启 Codex。
- Plugin Directory 中是否已经安装并启用 `Harness Powers`。
- Codex 配置文件（macOS/Linux：`~/.codex/config.toml`；Windows：`%USERPROFILE%\.codex\config.toml`）中对应 plugin 是否被设置为 `enabled = false`。

不要直接编辑已安装缓存中的 Superpowers（macOS/Linux 示例路径）：
...
```powershell
# 验证 plugin.json 可被正常解析
python3 -m json.tool plugins\harness-powers\.codex-plugin\plugin.json | Out-Null
if ($LASTEXITCODE -ne 0) { Write-Error "plugin.json 解析失败" }

python3 -m json.tool plugins\harness-powers\.cursor-plugin\plugin.json | Out-Null
if ($LASTEXITCODE -ne 0) { Write-Error "Cursor plugin.json 解析失败" }

# 列出所有 SKILL.md（预期 14 个）
Get-ChildItem -Path plugins\harness-powers\skills -Recurse -Depth 2 -Filter SKILL.md |
  Select-Object -ExpandProperty FullName | Sort-Object
```

预期结果（跨平台相同）：

- Codex 与 Cursor 两份 `plugin.json` 都能被正常解析。
...
```powershell
# 如果安装了 Git for Windows，可直接使用 Git Bash 运行：
bash tests/run-all.sh

# 或使用 WSL：
wsl bash tests/run-all.sh
```
...
**Windows（PowerShell，需已安装 ripgrep）：**

```powershell
$result = rg -n "docs/superpowers/(specs|plans)" plugins\harness-powers\skills
if ($result) { Write-Error "发现旧路径引用，请修正"; exit 1 }
```
...
- 不直接修改 Codex 插件缓存目录（macOS/Linux：`~/.codex/plugins/cache/openai-curated/superpowers`；Windows：`%USERPROFILE%\.codex\plugins\cache\openai-curated\superpowers`）。
...
- 每次修改后至少运行插件专项测试；涉及全仓规则时运行 `tests/run-all.sh`（Windows 下通过 Git Bash 或 WSL 执行）。
